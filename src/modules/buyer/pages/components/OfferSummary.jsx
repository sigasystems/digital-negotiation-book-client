  ...
  return (
    <Card className="shadow-lg border-slate-200 hover:shadow-xl transition-shadow pt-4">
      <CardHeader className="bg-gradient-to-r from-slate-50 to-blue-50 border-b p-4">
        ...
        <CardDescription>
          Original offer details and specifications
        </CardDescription>
      </CardHeader>
      <CardContent className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <SummaryItem icon={MapPin} label="Origin" value={`${offer.origin}`} />
        </div>
...
const SummaryItem = ({
  ...
  <div className="space-y-1">
    ...
    <p
      className={`text-sm ${isBold ? "font-semibold" : ""} ${isGreen ? "text-green-600" : "text-slate-900"}`}
    >
      {value}
    </p>
  </div>
