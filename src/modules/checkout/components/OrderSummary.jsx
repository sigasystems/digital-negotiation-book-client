...
import { Button } from "@/components/ui/button";
import { Separator } from "@/components/ui/separator";
import { createPayment, becomeBusinessOwner } from "../services/paymentService";
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.log("✅ Step 1: Register success", registerRes.data);
    ...
    console.log("✅ Step 2: Login success", tokenPayload.id);
    const user = tokenPayload;
    if (!user) throw new Error("Auto login failed");


    // 3️⃣ Create Stripe checkout session
    console.log("✅ Step 3: About to call createPayment()");
    const paymentPayload = {
      userId: user.id,
      planId: selectedPlan.id,
      billingCycle,
    ...
    const paymentRes = await createPayment(paymentPayload);
     ...
     console.log("✅ Step 4: Stripe session response", paymentRes);
    ...
    if (paymentRes?.url) {
      ...
      window.location.href = paymentRes.url;
          console.log("✅ Step 5: Business owner created succesfully...", ownerRes.data);

    
    //  4️⃣ Create Business Owner record before Stripe
    const ownerPayload = {
      planId: selectedPlan.id,
      billingCycle,
      userId: user.id,
        ...
        <Card className="shadow-lg border-slate-200">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span>Subtotal</span>
                <span>{calculateTotal()} {selectedPlan.currency}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span>Tax</span>
